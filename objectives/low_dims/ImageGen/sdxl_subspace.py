...
from ptp.sdxl_attention_editing_subspace_pipeline import SDXLAttentionEditingPipeline
...
def utility(X, opt, device=None, maximize=True):
    ...
    for i in range(X.shape[0]):
        ...
        print(" ")
        inp = X[i]
        pipe = SDXLAttentionEditingPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0",torch_dtype=dtype, use_safetensors=True,).to(device)
...
def record_choice(images, title=None,MAX_COLS=4, json_input=".json", orig_path=None, target=None):
    ...
    # port_val = int(os.getenv("GRADIO_SERVER_PORT", 7860))
...
def non_human_choice(images, prompt, scoring_metric="pickscore", opt=None, target=None, device="cuda"):
    ...
    print(" ")
    print("scores: ",scores)
    print(f"max-score, index: {max_score} , {best_index}")
    print(" ")
    return best_index
