...
from scripts.promptset.promptset_BObench.baselines.utils.demon.src.config import FILE_PATH, C_FILE_PATH, DTYPE, DEVICE, IMAGE_DIMENSION
...
class LatentConsistencyModel(LatentModel):
    ...
    def __init__(self, path=C_FILE_PATH, scheduler_path=FILE_PATH):
        super().__init__()
        unet = UNet2DConditionModel.from_pretrained(path, torch_dtype=DTYPE).to(device=DEVICE)
