...
import math
from utils_img_processing import *
import os
...
def create_controller(
    ...
    n_cross_replace = feat_attn_kwargs.get("n_cross_replace", 0.4)
    n_self_replace = feat_attn_kwargs.get("n_self_replace", 0.4)
    ...
    edit_start_step = int(feat_attn_kwargs["edit_start_step"] * num_inference_steps) if edit_start_step is not None else int(feat_attn_kwargs["n_self_replace"][0] * num_inference_steps)
    e_s = {}
    d_s = {}
...
class AttentionStore(AttentionControl):
    ...
    def get_empty_store():
        return {"down_cross": [], "mid_cross": [], "up_cross": [], "down_self": [], "mid_self": [], "up_self": []}
                # ,"down0_resnet": [],"down1_resnet": [],"down2_resnet": [], "mid_resnet": [], "up2_resnet": [], "up1_resnet": [], "up0_resnet": []}
...
class FeatandAttnControlEdit(AttentionStore, abc.ABC):
    ...
    def forward(self, attn, val, query, key, is_cross: bool, place_in_unet: str):
        super(FeatandAttnControlEdit, self).forward(attn, val, query, key, is_cross, place_in_unet[:-1] if place_in_unet != "mid" else place_in_unet)
        recomp = False
        if is_cross or (self.num_self_replace[0] <= self.cur_step < self.num_self_replace[1]):
            ...
            if is_cross:
                if self.should_edit_feat(place_in_unet,"cross"):
                    attn[1:] = self.replace_cross_attention(attn_base, attn_replace, place_in_unet)
                if self.should_edit_feat(place_in_unet,"c-value"):
                    val[1:] = self.edit_q_k_v(val_base, place_in_unet, feat_type="value", is_cross=True)
                if self.should_edit_feat(place_in_unet,"c-query"):
                    query[1:] = self.edit_q_k_v(query_base, place_in_unet, feat_type="query",is_cross=True)
                    recomp = True
                if self.should_edit_feat(place_in_unet,"c-key"):
                    key[1:] = self.edit_q_k_v(key_base, place_in_unet, feat_type="key", is_cross=True)
                    recomp = True
            else:
                ...
                if self.should_edit_feat(place_in_unet,"s-value"):
                    val[1:] = self.edit_q_k_v(val_base, place_in_unet, feat_type="value", is_cross=False)
                if self.should_edit_feat(place_in_unet,"s-query"):
                    query[1:] = self.edit_q_k_v(query_base, place_in_unet, feat_type="query",is_cross=False)
                    recomp = True
                if self.should_edit_feat(place_in_unet,"s-key"):
                    key[1:] = self.edit_q_k_v(key_base, place_in_unet, feat_type="key", is_cross=False)
                    recomp = True
    ...
    def __init__(
        ...
        if isinstance(self_replace_steps, float):
            self_replace_steps = self_replace_steps, self_replace_steps + delta
...
class FeatandAttnComposite(FeatandAttnControlEdit):
    ...
    def edit_resnet_features(self, feats, place_in_unet):
        if not self.should_edit_feat(place_in_unet, inp_type="resnet"):
            return feats
        if self.edit_start_step + self.delta < self.cur_step or self.cur_step < self.edit_start_step:
            return feats
...
################################ no mask ######################################
    ...
    def __init__(
        ...
        super().__init__(
            prompts, num_steps, cross_replace_steps, self_replace_steps,local_blend, tokenizer, device, attn_res, edit_blocks, per_word_cross_masks,
...
def get_params(blk,attn_modify_blocks):
    shift_cfg = {
        "down0": [(-0.55, -0.1),(0.55, 0.35)],
    "down1": [(-0.2, -0.1),(0.2, 0.1)],
    "down2": [(-0.2, -0.1),(0.2, 0.1)],
    "mid":   [(-0.2, -0.1),(0.2, 0.1)], 
    "up0":   [(-0.25, -0.05),(0.25, 0.25)],
    "up1": [(-0.45, -0.15),(0.6, 0.35)],
    "up2": [(-0.45, -0.15),(0.65, 0.45)],
    "down0-down1-up0": [(-0.3, -0.1),(0.3, 0.3)],
    "down2-mid-up0": [(-0.2, -0.1),(0.2, 0.1)],
    "down2-mid": [(-0.2, -0.1),(0.2, 0.1)]
    }
    scale_cfg = {
         "down0": [0.5,-0.5], #[0.5,-0.5]
        "down1": [0.25,-0.25], #(-0.4,0.3)
    "down2": [0.15,-0.15],
    "mid":   [0.25,-0.25], #(-0.2, 0.2)
    "up0":   [0.15,-0.15], #(-0.1, 0.16)
    "up1": [0.25,-0.25],
    "up2": [0.5,-0.5],
    "down0-down1-up0": [0.3,-0.3],
    "down2-mid-up0": [0.25,-0.25],
    "down2-mid": [0.25,-0.25]
    }
    reweight_cfg = {
            "down0": [0.25,-0.25], #[0.5,-0.5]
            "down1": [0.15,-0.15], #(-0.4,0.3)
        "down2": [0.15,-0.15],
        "mid":   [0.15,-0.15], #(-0.2, 0.2)
        "up0":   [0.15,-0.15], #(-0.1, 0.16)
        "up1": [0.25,-0.25],
        "up2": [0.5,-0.5],
        "down0-down1-up0": [0.3,-0.3],
        "down2-mid-up0": [0.15,-0.15],
        "down2-mid": [0.15,-0.15]
    }
    rotate_cfg = {
            "down0": [0.25,-0.25], #[0.5,-0.5]
    ...
    affine_cfg = {
        "down0": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "down1": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "down2": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "mid":   [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)], 
    "up0":   [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "up1": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "up2": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "down0-down1-up0": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "down2-mid-up0": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)],
    "down2-mid": [(-0.2, -0.2, 0.2, -0.3, 0.1, 0.3, -0.3),(0.2, 0.2, -0.2, 0.3, -0.1, -0.3, 0.3)]
    }

    operators = ["shift","scale","reweight","rotate"]#"shift","scale","reweight","rotate"]
    op_params = {}
    op_params["shift"] = {"shift_pos": shift_cfg[blk]}
    op_params["scale"] = {"scale_strengths": scale_cfg[blk]}
    op_params["reweight"] = {"equalizer_strengths": reweight_cfg[blk],}
    op_params["rotate"] = {"rotate_angles": rotate_cfg[blk],}
    op_params["affine"] = {"params": affine_cfg[blk]}

    print(op_params)
    
    relative_factor = {"cross":1.0,"self":1.0,"resnet":1.0, "value":1.0, "query":1.0, "key":1.0} #self between 0.75-1
    base_alphas = {
            "resnet": (0.9, 0.1),
            "self":   (0.8, 0.2),
            "value":  (0.8, 0.2),
    ...
    schedule = {"cross":"exp","self":"exp","resnet":"exp","value":"exp"},
    blending_alphas = {"base_alphas":base_alphas,"schedule":schedule,"weight_alphas_kwargs":weight_alphas_kwargs}
    pad_crop = False
    pad_kwargs = {"mode":'constant',"value":0}
    sr = 256
    edit_blocks = {}
    edit_blocks["resnet"] = blk.split("-") if "-" in blk else [blk]
    edit_blocks["self"] = attn_modify_blocks["self"]
    edit_blocks["value"] = attn_modify_blocks["self"]
    edit_blocks["cross"] = attn_modify_blocks["cross"]
    return operators, op_params, relative_factor, blending_alphas, pad_crop, sr, edit_blocks


# t_edit 0.1-0.2 or 0.02 
# delta = 0.1 or 0.2
# feat = resnet or conv2
# edit_blk_lst = ["down2-mid","down2-mid-up0","mid","down2","up0"]
# attn_modify_blocks = {"cross":["down2","mid"], "self":["mid","up0"]}
# d2-m : max_shift = 0.6*32, max_scale=3, max_angle=60, T=0.1 delta=0.1
# d2 : max_shift = 0.5*32, max_scale=3, max_angle=60, T=0.1 delta=0.1 - 3rd best
# d2-m conv2 : max_shift = 0.6*32, max_scale=3, max_angle=60, T=0.02 delta=0.1 - best
# d2-m-u0 conv2 : max_shift = 0.55*32, max_scale=3, max_angle=60, T=0.02 delta=0.1 - 4th best

## affine
# d2-m conv2 : max_shift = 0.3*32, max_scale=3, max_angle=30, shear=10, T=0.02 delta=0.2 - 2nd best