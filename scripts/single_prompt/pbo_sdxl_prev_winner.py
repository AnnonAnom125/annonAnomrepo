...
def start(opt: MultiBOConfig):
    ...
    comp = opt.edit_type
    opt.output_path = os.path.join(opt.output_path,"geometric","self-only","d-blk0-rest-blk0","single-mask","down2-mid-up0-up1",f"T-edit-{opt.t_edit}_Delta-{opt.delta}-{comp}")
    os.makedirs(opt.output_path,exist_ok=True)
    
    title = f"PROMPT= {opt.prompts} T-EDIT= {opt.t_edit} \
    DELTA= {opt.delta} IMG_SEED= {opt.img_seed} EDIT_TYPE= {opt.edit_type} BO_SEED= {opt.seed} \
    INIT_SAMPLES= {opt.num_initial_samples} INIT_COMP= {opt.m} ACF= {acfs[0]} NUM_TRIALS= {opt.num_trials} \
    NUM_BATCHES= {opt.num_batches} Q= {opt.q}"
    ...
    # train_comp, train_ch = gen_comp(train_y, opt.m, T=opt.T, noise=opt.noise, device=opt.device,title=opt.title,kwargs=kwargs_gen_comp)
    # mll, model = init_and_fit_model(train_X, train_comp, likelihood, ch=train_ch, device=opt.device,type_likelihood=opt.type_likelihood, multi=opt.multi)  
    ...
    for i in range(opt.num_trials):
        ...
        comparisons, choices = gen_comp(init_y, opt.m, T=opt.T, noise=opt.noise, device=opt.device,title=opt.title,kwargs=kwargs_gen_comp) #
        ...
        for j in range(1, opt.num_batches + 1):
            for algo, acf_f in zip(opt.acf_algos,acf_func_lst):
                ...
                # 2s-qEI
                    ...
                    opt.increment = False
                    acq_func = acf_f(pref_model=models[algo])
                elif "qEI" in algo:
                    opt.increment = False
                ...
                if algo != "rand" and algo != "2s-qEI":
                    ...
                    if "EUBO" in algo:
                        opt.increment = False
                        acq_func = acf_f(pref_model=models[algo],previous_winner=best_X)
