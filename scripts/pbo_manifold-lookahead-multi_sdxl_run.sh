...
bash ./scripts/pbo_manifold-lookahead-multi_sdxl.sh $gpu "$objectives" "$mode" "$algos" "$result_path" > logs/test-$mode-$objectives-pair-logit-$algos-$gpu-new-seed_0.log 2>&1 &
PID=$!
