...
from ptp.sd3_attention_editing_pipeline import SD3AttentionEditingPipeline
...
def utility(X, opt, device=None, maximize=True):
    ...
    for i in range(X.shape[0]):
        ...
        print(" ")
        inp = X[i]
        pipe = SD3AttentionEditingPipeline.from_pretrained("stabilityai/stable-diffusion-3.5-medium",torch_dtype=dtype, use_safetensors=True,).to(device)
...
def unedited_generation(prompt, seed, num_inference_steps, device):
    dtype = torch.float16 if device.type == "cuda" else torch.float32
    pipeline = StableDiffusion3Pipeline.from_pretrained("stabilityai/stable-diffusion-3.5-medium",torch_dtype=dtype).to(device)
