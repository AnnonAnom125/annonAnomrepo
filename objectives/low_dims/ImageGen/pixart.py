...
from ptp.pixart_attention_editing_pipeline import PixARTAttentionEditingPipeline
...
def utility(X, opt, device=None, maximize=True):
    ...
    for i in range(X.shape[0]):
        ...
        print(" ")
        inp = X[i]
        pipe = PixARTAttentionEditingPipeline.from_pretrained("PixArt-alpha/PixArt-Sigma-XL-2-1024-MS",torch_dtype=dtype, use_safetensors=True,).to(device)
        ...
        if opt.use_cross_attention_masks:
            for res in [64,32]:
                if not os.path.exists(os.path.join(cross_maps_path,f"per_word_cross_masks_seed-{seed}_res-{res}.pt")):
                    ...
                    print(" ")
                    image = pipe(prompts, num_inference_steps=opt.num_inference_steps, feat_attn_kwargs=feat_attn_kwargs_1, generator=gen, feat_blocks=feat_blocks, feats=feats, before_softmax=before_softmax, attn_uncond=attn_uncond, edit_resnet_uncond=edit_resnet_uncond)
        ...
        print(op_params)
        imgs = pipe(prompts, num_inference_steps=opt.num_inference_steps, feat_attn_kwargs=feat_attn_kwargs, generator=gen, feat_blocks=feat_blocks, feats=feats, before_softmax=before_softmax, attn_uncond=attn_uncond, edit_resnet_uncond=edit_resnet_uncond)['images']
        image = imgs[1]
        image_orig = imgs[0]
...
def unedited_generation(prompt, seed, num_inference_steps, device):
    pipeline = PixArtSigmaPipeline.from_pretrained("PixArt-alpha/PixArt-Sigma-XL-2-1024-MS", torch_dtype=torch.float16).to(device)
...
def record_choice(images, title=None,MAX_COLS=4, json_input=".json", orig_path=None, target=None):
    TOT = len(images)
