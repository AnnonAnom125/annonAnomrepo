...
class FLUXAttentionEditingPipeline(FluxPipeline):
    r"""
    Args:
    This model inherits from [`FluxPipeline`]. Check the superclass documentation for the generic methods the library implements for
    ...
    def register_feature_attention_control(self, controller):
        ...
        # These handle both text and image tokens in parallel
        for i, block in enumerate(self.transformer.transformer_blocks):
            place_in_unet = f"double_{i}" # Labeling for your controller
            
            block.attn.set_processor(FeatandAttnProcessor(
        ...
        # 2. Register processors for SingleStream Blocks (38 blocks)
        # These process text and image tokens as a single unified sequence
        for i, block in enumerate(self.transformer.single_transformer_blocks):
            place_in_unet = f"single_{i}"
        ...
        for name, module in self.transformer.named_modules():
            # In FLUX, 'ff' (Feed-Forward) is the closest equivalent to SDXL's ResNets
            if "ff.net.2" in name: 
                key_resnet = name.partition("ff.net.2")
                i = name.split(".")[1]
                place_in_unet = f"double_{i}"
