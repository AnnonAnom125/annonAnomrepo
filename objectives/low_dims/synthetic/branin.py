...
def utility(X: Tensor, device = 'cpu', maximize=True) -> Tensor:
    ...
    Returns:
        `-B(X)`, the negative value of the standard Branin function if maximize is set
    """
    ...
    if maximize:
        ...
        result = B
    return result.to(device) if batch else result.squeeze(0).to(device)

def optima(dims):
    return torch.Tensor(GLOBAL_MAXIMUM).item()