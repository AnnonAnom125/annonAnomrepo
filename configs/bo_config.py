...
class MultiBOConfig_SDXL:
    ...
    dbs_mode = "spectral" # spectral, energy
    ...
    t_edit: float = 0.0
    delta: float = 0.2
    extra_kwargs: Dict[str, Dict] = field(default_factory=lambda: {"delta":{"resnet":0.1}}) #"delta":{"resnet":0.1}
    ...
    attn_blk: Dict[str, List[str]] = field(default_factory=lambda: {"cross":[], "self":[], 
        "s-query":["up0","up1"], "s-key":["up0","up1"], "s-value":["up0","up1"], "c-value":[], "c-query":[], "c-key":[]}) #PixART:f"block_{i}" for i in range(3,14) #SDXL:{"cross":[], "self":[], "s-value":[], "c-value":[], "s-query":["up0","up1"], "c-query":[], "s-key":["up0","up1"], "c-key":[]}    #{"cross":["down2","mid"], "self":["mid","up0"]} #"down2","mid","up0","up1"
    attn_res: int = 32
    ...
    sr: int = 128
    _alpha: float = 1*torch.pi/6 #12   #2/3
    _s_alpha: float = 1*torch.pi/12
    _t: float = 0.25  #0.25   #0.15  #0.75
    _s: float = 0.5 #0.5     #0.25  #0.75
    _t_hom: float = 0.4
    _t_tps:float = 0.4   #0.4    #0.2   #0.75
...
@dataclass
class MultiBOConfig_SDXL_promptset(MultiBOConfig_SDXL):
...
# img_seeds = 5994, 1688, 9859, 9160, 6400, 8981, 2707, 7161, 1705, 7818
