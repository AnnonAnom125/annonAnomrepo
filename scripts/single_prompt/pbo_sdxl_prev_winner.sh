...
PROMPTS="a couch , a table and a lamp"
...
NUM_TRIALS=1
NUM_BATCHES=20
DIM=24 #composite = 10, affine = 14, geo(a) = 48 geo(b) = 24
...
# 0.02 0.1
# 0.1 0.2
for T_EDIT in 0.1; do
    for DELTA in 0.1; do
        for RES_BLK in "a"; do #"down2-mid-up0-up1"
            echo "T_EDIT = $T_EDIT"
            echo "DELTA = $DELTA"
            echo "RES_BLK = $RES_BLK"
            echo "Running job for $OBJECTIVES"
            python -u scripts/pbo_sdxl_prev_winner.py \
