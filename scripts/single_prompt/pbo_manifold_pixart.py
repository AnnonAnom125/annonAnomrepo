...
from configs import MultiBOConfig_PixART
...
def create_folder_path(opt):
    ...
    if opt.obj_name in opt.image_models:
        pp = os.path.join(pp,f"{sanitize_filename(opt.prompts[:200])}",f"seed-{opt.img_seed}")
    return pp

def sanitize_filename(text):
    return re.sub(r'[^\w\-_\. ]', '', text).replace(' ', '_')[:50]

@pyrallis.wrap()
def start(opt: MultiBOConfig_PixART):
    ...
    opt.output_path = os.path.join(opt.output_path,"acf-fixed-new",convert_t,opt.edit_type,"single-mask",f"{opt.feat_blocks}-blk",f"res-{opt.res_blk}--{ss}",f"T-edit-{opt.t_edit}_Delta-{opt.delta}-{comp}")
