...
from configs import MultiBOConfig_SDXL
...
def create_folder_path(opt):
    ...
    if opt.obj_name in opt.image_models:
        pp = os.path.join(pp,f"{sanitize_filename(opt.prompts)}",f"seed-{opt.img_seed}")
    return pp
...
@pyrallis.wrap()
def start(opt: MultiBOConfig_SDXL):
    ...
    opt.output_path = os.path.join(opt.output_path, f"trial_seed-{opt.random_seeds[0]}","best-best-model","dbs-final",convert_t,opt.edit_type,"single-mask",f"{opt.feat_blocks}-blk",f"attn_uncond-{opt.attn_uncond}","non-human",f"res-{opt.res_blk}--{ss}",f"T-edit-{opt.t_edit}_Delta-{opt.delta}-{comp}")
