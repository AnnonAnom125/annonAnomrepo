...
class Selector():
    ...
    def __init__(self, device, model_path):
        ...
        if not os.path.exists(model_path):
            print('Downloading AES Model ...')
            url = ' https://github.com/christophschuhmann/improved-aesthetic-predictor/blob/main/sac%2Blogos%2Bava1-l14-linearMSE.pth'
            r = requests.get(url, stream=True)
            with open(model_path, 'wb') as AES:
                total_length = int(r.headers.get('content-length'))
                for chunk in progress.bar(r.iter_content(chunk_size=1024), expected_size=(total_length/1024) + 1): 
                    if chunk:
                        AES.write(chunk)
                        AES.flush()
            print('Download AES Model to {} sucessfully.'.format(model_path))
