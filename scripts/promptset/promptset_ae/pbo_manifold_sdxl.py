...
def run_pbo(opt: MultiBOConfig_SDXL_promptset):
    ...
    for i in range(opt.num_trials):
        if i == 0 and opt.prompts == "a elephant and a rabbit" and opt.img_seed == 7876:
            res = [Image.open("/home/exx/Documents/raja/MultiBO/outputs/promptset/promptset_ae/MultiBO/low-image/sdxl/animals/Multiwise/Logit-qEI/a elephant and a rabbit/seed-7876/geometric/first-blk/res-n--s-query_up0-up1--s-key_up0-up1--s-value_up0-up1/T-edit-0.0_Delta-0.2-/qEI/best_image_trial-0_batch-20.png")]
            print(res)
        else:
                ...
                for algo, acf_f in zip(opt.acf_algos,acf_func_lst):
                    if j == 1:
                        ...
                        if opt.obj_name not in opt.image_models:
                            ...
                            best_X, best_image = find_best_image(init_X, init_y,title=opt.title,json_input=opt.json_input,orig_path=orig_path,kwargs={"obj":opt.obj,"opt":opt})
                    ...
                    if opt.obj_name not in opt.image_models:
                        ...
                        # max_y = utility(X, opt)
                        best_X, max_image = find_best_image(X,y,title=opt.title,json_input=opt.json_input,orig_path=orig_path,kwargs={"obj":opt.obj,"opt":opt})
