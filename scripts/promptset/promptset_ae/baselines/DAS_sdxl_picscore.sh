...
for category in $CATEGORY; do
    ...
    python -u scripts/promptset/promptset_ae/baselines/DAS_sdxl.py \
    --output_path "$RESULT_PATH" --mode $MODE --obj_name "$OBJECTIVES" --num_samples_per_prompt 3 \
    --prompts_per_category 5 --promptset_path $PROMPTSET_PATH --use_random_seeds $USE_RAND_SEED --reward_type "picscore" \
