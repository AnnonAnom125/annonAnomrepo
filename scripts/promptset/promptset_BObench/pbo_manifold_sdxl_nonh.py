...
def start(opt: MultiBOConfig_SDXL_promptset):
    ...
    for category in prompt_categories:
        ...
        for i, prompt_idx in enumerate(prompts_ids):
            ...
            if opt.score_metric == "aesthetic" and i < 1:
...
def run_pbo(opt: MultiBOConfig_SDXL_promptset, target_img=None, tar_seed=None):
    ...
    for i in range(opt.num_trials):
        if i < 2 and opt.prompts=="An airplane on the runway of an airport.":
