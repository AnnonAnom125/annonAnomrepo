...
from utils.obj_utils import get_objective_subspace, convert_t_way_to_pairwise
    ...
    for category in prompt_categories:
        ...
        for i, prompt_idx in enumerate(prompts_ids):
            ...
            if category == "part2" and i < 3:
                ...
                _, _, obj_name = get_objective_subspace(opt.obj_name,opt.mode)
                ...
                image, ref_paths = run_sub(opt, target_img, tar_seed)
...
def run_sub(opt: MultiBOConfig_SDXL_promptset, target_img=None, tar_seed=None):
    opt.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    opt.seed = opt.init_bo_seed
    set_all_seeds(opt.seed)
    # initialize modules
    utility, optimum, obj = get_objective_subspace(opt.obj_name,opt.mode)
    if opt.obj_name not in opt.image_models:
    ...
    for i in range(opt.num_trials):
        if i < 1 and opt.prompts=="An airplane on the runway of an airport.":
            result_final.append(Image.open(os.path.join(opt.output_path,opt.acf_algos[0],f"best_image_trial-{i}_batch-{opt.num_batches}.png")))
            ref_final.append(os.path.join(opt.output_path,"image_orig","image_orig.png"))
