...
def start(opt: MultiBOConfig_SDXL_promptset):
    ...
    for category in prompt_categories:
        ...
        for i, prompt_idx in enumerate(prompts_ids):
            ...
            img_seed = int(prompt_idx["target_seed"])
            tar_seed = int(prompt_idx["edit_seed"])
            ...
            print(" ")
            if prompt == "The fragrant flowers bloomed on the sturdy stem and the thorny bush." and img_seed == 320923:
...
def run_pbo(opt: MultiBOConfig_SDXL_promptset, target_img=None, tar_seed=None):
    ...
    opt.attn_blk = {"cross":[], "self":[], 
        "s-query":["up0","up1"], "s-key":[], "s-value":[], "c-value":[], "c-query":[], "c-key":[]}
    opt._alpha= 1*torch.pi/18 
    opt._s_alpha= 1*torch.pi/18
    opt._t= 0.15  
    opt._s= 0.1 
    opt._t_tps = 0.1
