...
bash ./scripts/promptset/promptset_cocoee/pbo_manifold-dbs-multi_sd3.sh $gpu1 "$objectives" "$mode" "$algos" "$result_path" "$category1" > logs/test-$mode-$objectives-$category1-pair-logit-$algos-$gpu1.log 2>&1 &
PID1=$!
